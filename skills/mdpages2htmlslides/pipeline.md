...
| L2 job | Clone |
|---|---|
| cover | `templates/TIANSIGHT/layouts.html` `[data-layout=cover]` |
| chapter | gold `.slide.divider` or TIANSIGHT cover with act number only |
| chart / chart-table | `[data-layout=viz-full]` / `viz-table` |
| matrix | `[data-layout=matrix-full]` |
| kpi | `[data-layout=kpi-grid]` |
| roster | `[data-layout=roster]` |
| compare | `[data-layout=viz-duo]` |
| verdict | `[data-layout=verdict]` |
| toc / readme / statement | `.slide` body: header + chips + `.srcbar` + `.body` + `.takebar` |

Gold museum for fig SVG recipes: `ref/清水亭_产品结构诊断_TIANSIGHT幻灯片 (5).html` (47 `.figslide`). Copy geometry, not its missing takeaway bars.

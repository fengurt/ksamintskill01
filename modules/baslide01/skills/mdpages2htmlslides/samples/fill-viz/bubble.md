...
Clone `templates/TIANSIGHT/jobs/chart.html`. `viewBox="0 0 1170 500"`.

- Radius = `8 + 14 * sqrt(size / max)` (cap 22). Never linear `14 + 28 * (s/smax)`.
- Paint large circles first, labels last. Highlight uses `--sd-secondary`.
- Dashed crosshairs at the median; high side includes equals `≥`.
- At most 12 labels, thinned when centers sit inside ~72px. Paper-stroke halo on type.
- Caption: `n= · 面积∝√量 · 虚线=中位 ≥ · 标 k 家，其余见清单`.
- Next page is a roster 清单 at brand grain. Do not pretend the labeled set is the full series.
